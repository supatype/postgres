...
PG_MODULE_MAGIC_EXT(.name = "pg_guard", .version = MODVERSION);
#else
PG_MODULE_MAGIC;
#endif
...
static bool pg_guard_needs_fmgr_hook(Oid functionId) {
...
static void pg_guard_fmgr_hook(FmgrHookEventType event, FmgrInfo *flinfo,
                                Datum *private) {
  switch (event) {
...
static void pg_guard_hook(PROCESS_UTILITY_PARAMS) {
  ...
  case T_AlterRoleStmt: {
    ...
    switch_to_superuser(pg_guard_superuser, &already_switched_to_superuser);
  ...
  case T_CreateRoleStmt: {
    if (IsTransactionState() && !superuser()) {
      ...
      if (is_current_role_privileged()) {
        ...
        switch_to_superuser(pg_guard_superuser,
...
static bool extensions_parameter_overrides_check_hook(
  ...
  if (val) {
    ...
    if (state.error_msg) {
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg("pg_guard.extensions_parameter_overrides: %s",
                             state.error_msg)));
    }
...
static bool policy_grants_check_hook(char                            **newval,
  ...
  if (val) {
    ...
    if (state.error_msg) {
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg("pg_guard.policy_grants: %s", state.error_msg)));
    }
    ...
    if (state.error_msg) {
      ereport(ERROR,
              (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
               errmsg("pg_guard.drop_trigger_grants: %s", state.error_msg)));
    }
...
reserved_roles_check_hook(char **newval, __attribute__((unused)) void **extra,
                          __attribute__((unused)) GucSource source) {
  check_parameter(*newval, "pg_guard.reserved_roles");
...
reserved_memberships_check_hook(char                            **newval,
                                __attribute__((unused)) void    **extra,
                                __attribute__((unused)) GucSource source) {
  check_parameter(*newval, "pg_guard.reserved_memberships");
...
static bool placeholders_disallowed_values_check_hook(
    char **newval, __attribute__((unused)) void **extra,
    __attribute__((unused)) GucSource source) {
  check_parameter(*newval, "pg_guard.placeholders_disallowed_values");
...
privileged_extensions_check_hook(char                            **newval,
                                 __attribute__((unused)) void    **extra,
                                 __attribute__((unused)) GucSource source) {
  check_parameter(*newval, "pg_guard.privileged_extensions");
...
static bool privileged_role_allowed_configs_check_hook(
    char **newval, __attribute__((unused)) void **extra,
    __attribute__((unused)) GucSource source) {
  check_parameter(*newval, "pg_guard.privileged_role_allowed_configs");
...
constrained_extensions_assign_hook(const char                   *newval,
  ...
  if (newval) {
    ...
    if (state.error_msg) {
      ereport(ERROR, (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
                      errmsg("pg_guard.constrained_extensions: %s",
                             state.error_msg)));
    }
  }
}
...
static bool placeholders_check_hook(char                            **newval,
  ...
  if (val) {
    ...
    if (!SplitIdentifierString(pstrdup(val), ',', &comma_separated_list))
      EREPORT_INVALID_PARAMETER("pg_guard.placeholders");
    ...
    if (!saw_sep)
      ereport(ERROR,
              (errcode(ERRCODE_INVALID_PARAMETER_VALUE),
               errmsg("pg_guard.placeholders must contain guc placeholders")));
  }
...
void _PG_init(void) {
  ...
  ProcessUtility_hook = pg_guard_hook;

  // fmgr hook
  next_needs_fmgr_hook = needs_fmgr_hook;
  needs_fmgr_hook      = pg_guard_needs_fmgr_hook;

  next_fmgr_hook = fmgr_hook;
  fmgr_hook      = pg_guard_fmgr_hook;

  DefineCustomStringVariable(
      "pg_guard.extensions_parameter_overrides",
  ...
  DefineCustomStringVariable(
      "pg_guard.superuser",
      "Superuser to install extensions in pg_guard.privileged_extensions as",
      NULL, &pg_guard_superuser, NULL, PGC_SIGHUP, 0, NULL, NULL, NULL);

  // TODO emit a warning when this deprecated GUC is used
  DefineCustomStringVariable(
      "pg_guard.privileged_extensions_superuser",
      "Superuser to install extensions in pg_guard.privileged_extensions "
      "as. Deprecated: use pg_guard.superuser instead.",
      NULL, &pg_guard_superuser, NULL, PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomStringVariable(
      "pg_guard.privileged_role",
      "Non-superuser role to be granted with some superuser privileges", NULL,
      &privileged_role, NULL, PGC_SIGHUP, 0, NULL, NULL, NULL);

  DefineCustomStringVariable(
      "pg_guard.privileged_role_allowed_configs",
  ...
  DefineCustomBoolVariable("pg_guard.disable_program", NULL, NULL,
  ...
  EmitWarningsOnPlaceholders("pg_guard");
}
