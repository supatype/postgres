...
To make pg_guard available to the whole cluster, you can add the following to `postgresql.conf` (use `SHOW config_file` for finding the location).

```
shared_preload_libraries = 'pg_guard'
pg_guard.privileged_role = 'your_privileged_role'
```

Or to make it available only on some PostgreSQL roles use `session_preload_libraries`.

```
ALTER ROLE role1 SET session_preload_libraries TO 'pg_guard';
```
...
The privileged role is a proxy role for a SUPERUSER, which is configured by `pg_guard.superuser` (defaults to the bootstrap user, i.e. the role used to start the Postgres cluster).

When the privileged role creates a superuser-only database object (like publications):

- pg_guard will switch the role to the `pg_guard.superuser`, allowing the operation and creating the database object.
...
- Finally, pg_guard will switch back to the privileged role.
...
The skipping behavior can be logged by setting the `pg_guard.log_skipped_evtrigs` config to true, this is false by default.
...
```
pg_guard.privileged_role_allowed_configs="session_replication_role"
```

Some extensions also have their own superuser settings with a prefix, those can be configured by:

```
pg_guard.privileged_role_allowed_configs="ext.setting, other.nested"
```

You can also choose to allow all the extension settings by using a wildcard:

```
pg_guard.privileged_role_allowed_configs="ext.*"
```
...
pg_guard allows you to let non-superusers manage extensions that would normally require being a superuser. e.g. the `hstore` extension creates a base type, which requires being a superuser to perform.

To handle this, you can put the extension in `pg_guard.privileged_extensions`:

```psql
pg_guard.privileged_extensions = 'hstore'
```

Once you do, the extension creation will be delegated to the configured `pg_guard.superuser`. That means the `hstore` extension would be created as if by the superuser.
...
If you don't want to enable this functionality, simply leave `pg_guard.privileged_extensions` empty. Extensions **not** in `pg_guard.privileged_extensions` would behave normally, i.e. created using the current role.

### Extension Custom Scripts

pg_guard also lets you set custom scripts per extension that gets run at certain events. Currently supported scripts are `before-create` and `after-create`.

To make this work, configure the setting below:

```
pg_guard.extension_custom_scripts_path = '/some/path/extension-custom-scripts'
```
...
```
pg_guard.constrained_extensions = '{"plrust": {"cpu": 16, "mem": "1 GB", "disk": "500 MB"}, "any_extension_name": { "mem": "1 GB"}}'
```

The `pg_guard.constrained_extensions` is a json object, any other json type will result in an error.
...
```
pg_guard.extensions_parameter_overrides = '{ "pg_cron": { "schema": "pg_catalog" } }'
```
...
With pg_guard, this can be done like so:

```
pg_guard.policy_grants = '{ "my_role": ["public.not_my_table", "public.also_not_my_table"] }'
```
...
```
pg_guard.drop_trigger_grants = '{ "my_role": ["public.not_my_table", "public.also_not_my_table"] }'
```
...
```
pg_guard.reserved_roles = 'connector, storage_admin'
```
