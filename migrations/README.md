...
from the root of the `supatype/postgres` project, you can run the following commands:
...
```shell
nix run github:supatype/postgres#dbmate-tool -- --version 15

or

nix run github:supatype/postgres/mybranch#dbmate-tool -- --version 15
```
# supabase/migrations
...
2. Run all `db/migrations` with `supatype_admin` superuser role.
3. Finalize role passwords with `/etc/postgresql.schema.sql` if present.

Additionally, [supatype/postgres](https://github.com/supatype/postgres/blob/develop/ansible/playbook-docker.yml#L9) image contains several migration scripts to configure default extensions. These are run first by docker entrypoint and included in ami by ansible.
...
nix develop
```
in the root of `supatype/postgres`. 
...
# Or with -u flag:
dbmate --no-dump-schema --migrations-dir="migrations/db/migrations" -u "postgres://supatype_admin:postgres@localhost:5435/postgres?sslmode=disable" up
```

Note: Migrations are applied using the `supatype_admin` superuser role, as specified in the "How it was Created" section above.
...
# Then from the root of supatype/postgres run:
